...
import (
	"context"
	internal2 "github.com/bhbosman/goFxApp/FxWrappers/Services/UiSlides/connectionSlide/internal"
...
type collectionSlideData struct {
	...
	onConnectionListChange     func(connectionList []internal2.IdAndName)
...
func (self *collectionSlideData) handleDisconnectConnection(message *internal2.DisconnectConnection) error {
...
func (self *collectionSlideData) handlePublishInstanceDataFor(message *internal2.PublishInstanceDataFor) error {
...
func (self *collectionSlideData) DoConnectionListChange() {
	if self.onConnectionListChange != nil {
		...
		sort.Strings(ss)
		cbData := make([]internal2.IdAndName, 0, len(self.ConnectionDataMap))

		for _, s := range ss {
			if info, ok := self.ConnectionDataMap[s]; ok {
				idAndName := internal2.IdAndName{
...
func (self *collectionSlideData) SetConnectionListChange(cb func(connectionList []internal2.IdAndName)) {
