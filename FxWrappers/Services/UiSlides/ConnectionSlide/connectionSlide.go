...
type ConnectionSlide struct {
	data           internal2.IConnectionData
...
func (self *ConnectionSlide) SetConnectionListChange(list []internal2.IdAndName) {
...
func (self *ConnectionSlide) init() {
	...
	self.connectionList.SetChangedFunc(func(index int, mainText string, secondaryText string, shortcut rune) {
		_, _ = ISendMessage.CallISendMessageSend(self.ctx, self.channel, false, &internal2.PublishInstanceDataFor{
			Id:   mainText,
			Name: secondaryText,
		})
	})
	...
	self.actionList.AddItem("Disconnect", "", 0, func() {
		...
		_, _ = ISendMessage.CallISendMessageSend(self.ctx, self.channel, false,
			internal2.NewDisconnectConnection(text, secondary))
