...
import (
	...
	internal2 "github.com/bhbosman/goFxApp/FxWrappers/Services/UiSlides/FxServicesSlide/internal"
		...
		[]ChannelHandler.ChannelHandler{
			{
				...
				Cb: func(next interface{}, message interface{}) (bool, error) {
					if unk, ok := next.(internal2.IFxManagerSlide); ok {
						return internal2.ChannelEventsForIFxManagerSlide(unk, message)
					}
					return false, nil
				},
			},
		...
		for _, action := range data.Actions {
			if action == StopServiceString {
				self.actionList.AddItem(action, "", 0, func() {
					_, _ = internal2.CallIFxManagerSlideStopService(self.ctx, self.channel, false, data.Name)
					self.app.SetFocus(self.table)
				})
				continue
			}
			if action == StartServiceString {
				self.actionList.AddItem(action, "", 0, func() {
					_, _ = internal2.CallIFxManagerSlideStartService(self.ctx, self.channel, false, data.Name)
					self.app.SetFocus(self.table)
				})
				continue
			}
			if action == StartAllServiceString {
				self.actionList.AddItem(action, "", 0, func() {
					_, _ = internal2.CallIFxManagerSlideStartAllService(self.ctx, self.channel, false)
					self.app.SetFocus(self.table)
				})
				continue
			}
			if action == StopAllServiceString {
				self.actionList.AddItem(action, "", 0, func() {
					_, _ = internal2.CallIFxManagerSlideStopAllService(self.ctx, self.channel, false)
					self.app.SetFocus(self.table)
				})
				continue
			}
...
func NewFxServiceSlide(applicationContext context.Context, pubSub *pubsub.PubSub, app *tview.Application,
	fxManagerService FxAppManagerService.IFxManagerService) *FxServicesManagerSlide {
